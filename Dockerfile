...
# See https://github.com/msgrfinance/wasmvm/releases
...
RUN addgroup msgrc \
    && adduser -G msgrc -D -h /msgrc msgrc

WORKDIR /Msgrc

COPY --from=go-builder /code/build/msgrcd /usr/local/bin/msgrcd
...
CMD ["/usr/local/bin/msgrcd", "version"]
